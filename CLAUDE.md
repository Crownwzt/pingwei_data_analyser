...
│   ├── cleaning.py           ① 数据清洗     → outputs/01_cleaning.html
...
1. **预测目标**：实时统一结算点电价(元/MWh)
2. **数据范围**：仅使用 2025 年数据，2026 数据完全忽略
3. **数据切分**：时序严格按年月，**禁止随机打乱**
   - 训练集：2025-01 ~ 10（≥ 80% 占比）
   - 验证集：2025-11
   - 测试集：2025-12
4. **模型评估指标**：MAE、MSE、RMSE、MAPE
5. **数据泄漏**：`实时节点电价(元/MWh)` 与目标同时刻产生，必须显式列入黑名单
6. **day-ahead 合法性**：
   - 所有 `_实际` 后缀列在预测时刻未知，禁用
