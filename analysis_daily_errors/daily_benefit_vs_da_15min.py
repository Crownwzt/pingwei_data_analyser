"""
按"XGB相对日前价的收益"（= sum|日前-真实| - sum|XGB-真实|）重排测试集逐日图。

收益逻辑：
  - 日前价误差面积 = sum|日前价-真实价|
  - XGB 误差面积    = sum|XGB预测-真实价|
  - 收益 = 日前误差 - XGB误差
    · 正数：XGB 比日前更准，"赚"了（误差更小）
    · 负数：XGB 比日前更差，"亏"了（误差更大）
...
产物（全部写入本目录）：
  analysis_daily_errors/
    ├── daily_benefit_vs_da_15min.py        本脚本
    ├── daily_by_benefit/                   按收益排序（rank01_最赚 → rank29_最亏）
    │   └── rank01_Benefit{+XXX.X}_2025-MM-DD.png
    ├── summary_benefit.csv                 每日收益汇总
    └── ranking_comparison_benefit.png      排序对比
...
OUT_DIR = os.path.join(ROOT, "daily_by_benefit")
...
def daily_summary(hourly: pd.DataFrame) -> pd.DataFrame:
    ...
    for d, g in hourly.groupby("date"):
        xgb_errs = g["abs_err"].to_numpy()  # |XGB预测 - 真实|
        da_errs = (g["da"] - g["y_true"]).abs().to_numpy()  # |日前 - 真实|

        xgb_total = float(xgb_errs.sum())
        da_total = float(da_errs.sum())
        benefit = da_total - xgb_total  # 正=XGB更准(赚), 负=XGB更差(亏)

        mapes = g["mape"].to_numpy()
        rows.append({
            "date": d,
            "n": len(g),
            "mae": float(xgb_errs.mean()),
            "trmae10": _trmae(xgb_errs, 0.10),
            "xgb_total_error": xgb_total,
            "da_total_error": da_total,
            "benefit": benefit,  # XGB相对日前的收益
            "max_abs_err": float(xgb_errs.max()),
...
def plot_one_day(hourly_day: pd.DataFrame, fifteen_day: pd.DataFrame,
    ...
    # 收益标注：正数用绿色(赚)，负数用红色(亏)
    benefit_val = meta['benefit']
    benefit_color = '#28a745' if benefit_val >= 0 else '#dc3545'
    benefit_text = f"相对日前收益={benefit_val:+.1f}元" if benefit_val >= 0 else f"相对日前收益={benefit_val:.1f}元"
    ax1.set_title(
        f"[test] {meta['date']}  日内 24 点预测  "
        f"{benefit_text}  "
        f"XGB误差={meta['xgb_total_error']:.1f}  日前误差={meta['da_total_error']:.1f}",
        fontsize=11,
    )
...
def plot_ranking_comparison(daily: pd.DataFrame, out_path: str) -> None:
    d = daily.copy()
    d["rank_benefit"] = d["benefit"].rank(ascending=False).astype(int)
    d["rank_mae"] = d["mae"].rank(ascending=False).astype(int)
    d = d.sort_values("date")

    fig, ax = plt.subplots(figsize=(11, 5))
    x = np.arange(len(d))
    w = 0.4
    ax.bar(x - w / 2, d["rank_mae"], w, label="rank by MAE", color="#337ab7")
    ax.bar(x + w / 2, d["rank_benefit"], w, label="rank by 相对收益", color="#28a745")
    ax.set_xticks(x)
    ax.set_xticklabels([str(dd)[-5:] for dd in d["date"]], rotation=45, fontsize=8)
    ax.set_ylabel("排名 (数字越小 = 收益越大)")
    ax.set_title("两种排序对比：MAE vs 相对日前价的收益")
    ax.invert_yaxis()
...
def main():
    ...
    print("[3/4] 计算每日汇总并按 Benefit 排序")
    daily = daily_summary(hourly)
    daily.to_csv(os.path.join(ROOT, "summary_benefit.csv"),
                 index=False, encoding="utf-8-sig")
    print(daily[["date", "n", "mae", "benefit", "xgb_total_error", "da_total_error"]].to_string(index=False))
    ...
    for i, row in daily.iterrows():
        ...
        # 文件名：正数用+号，负数自带-号
        sign = "+" if row['benefit'] >= 0 else ""
        fname = f"rank{rank:0{rank_pad}d}_Benefit{sign}{row['benefit']:06.1f}_{date}.png"
        out = os.path.join(OUT_DIR, fname)
        plot_one_day(hourly_day, f15_day, row.to_dict(), out)

    plot_ranking_comparison(daily, os.path.join(ROOT, "ranking_comparison_benefit.png"))
    print("\n完成。")
